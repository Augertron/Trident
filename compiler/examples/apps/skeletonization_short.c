...
static inline char Fmin(char array[N][N], int x, int y) __attribute__((always_inline));
...
static inline char Fmax(char array[N][N], int x, int y) __attribute__((always_inline));
static inline char Fmax(char array[N][N], int x, int y) {
  char max = -126;
  int i, j;
  for(i=0; i<x; i++) {
    for(j=0; j<y; j++) {
